...
class processing_dataframe(CashFlowSimulator):
    ...
    def calculate_interest_payments(self, df):
        ...
        list_purchase_years = list(df[df['year_last_purchase'] > 0]['year_last_purchase'].unique())
        
        list_purchase_years.append(46)
        year_first_purchase  = int(df[df['year_last_purchase']!=0]['year_last_purchase'].min())
        ...
        for i in range(len(list_purchase_years) - 1):
            ...
            list_interest_payments.extend(interest_payments[start_year-1:end_year-1])
    ...
    def calculate_static_values(self, df):
        ...
        first_year_mortgage_payments = int(df[df['year_last_purchase']!=0]['year_last_purchase'].min())
        ...
        df['loss_rent_empty_house'] = np.where(df['year'].isin(years_begining_renting_out),
        ...
        df['purchase_costs'] = np.where(df['year'] < 3,
                                                    purchase_cost_future,
                                                    np.array(purchase_cost_future) + np.array(property_tax))
        
        df['purchase_costs'] = np.where(df['year'].isin(years_purchasing_house),
                                         df['purchase_costs'],
                                         0)
        
        df['selling_costs'] = np.round(np.where(df['year'].isin(years_selling_house),
                                       self.selling_cost_percentage*df['house_value_over_years'],
                                       0))
        
        
        df['mortgage_payments'] = np.where((df['year'] > last_year_mortgage_payments) | (df['year'] < first_year_mortgage_payments),
                                           0,
                                           df['year_last_purchase'].apply(lambda x: self.calculate_mortgage_payments_single_value(x)))
        
        
        df['interest_payments'] = self.calculate_interest_payments(df)
        
        df['equity_accumulation'] = np.where(df['year'] < first_year_mortgage_payments,
                                             0,
        ...
        df['cash_flow_renting_out'] = df['renting_out_income'] - df['loss_rent_empty_house'] - df['mortgage_payments']

        df['renting_cashflow'] = df['rent_expenses'] + df['energy_bills_rent']
        
        df['house_purchasing_cashflow'] = df['mortgage_payments'] + df['energy_bills'] + df['ground_lease_expenses'] + df['manteinance_cost'] + df['service_costs'] - df['airbnb_income'] + df['purchase_costs']
