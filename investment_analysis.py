...
def calculate_roe(transactions, updated_parameters_dict):
    ...
    df = df.set_index("year")
    df["actualized_final_cashflow"] = np.where(
        df["final_cashflow"] > 0,
        df["discount_rate"]
        / df.loc[horizon_year, "discount_rate"]
        * df["final_cashflow"],
        df["discount_rate"] * df["final_cashflow"],
    )

    initial_investment = sum(
        x for x in df.loc[:horizon_year, "actualized_final_cashflow"] if x < 0
    )
    net_investment_cash_return = sum(
        x for x in df.loc[:horizon_year, "actualized_final_cashflow"] if x > 0
    )
    ...
    if initial_investment != 0:
        total_roe_pct = round(
            100
            * (
                (net_investment_cash_return / -(initial_investment))
                ** (1 / horizon_year)
            )
            - 100,
            2,
...
def calculate_total_cashoutflow(transactions, updated_parameters_dict):
    ...
    df["cash_outflow"] = np.where(df["final_cashflow"] < 0, df["final_cashflow"], 0)
...
def calculate_annual_housing_cost(transactions, updated_parameters_dict):
    ...
    monthly_housing_cost = round(
        df.loc[:evaluation_year, "house_purchasing_cashflow"].mean() / 12
    )
...
def calculate_mortgage_payments_updated(transactions, updated_parameters_dict):
    ...
    monthly_mortgage_payment = round(
        df.loc[:evaluation_year, "mortgage_payments"].mean() / 12
    )
...
def calculate_equity_accumulation_update(transactions, updated_parameters_dict):
    ...
    monthly_equity_accumulation = round(
        df.loc[:evaluation_year, "equity_accumulation"].mean() / 12
    )
